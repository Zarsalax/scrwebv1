...
from scraper import scraper_loop, get_scraper_stats
...
<body>
    <div class="container">
        <div class="header">
            <div class="logo">💳</div>
            <h1>Team RedCards</h1>
            <p class="subtitle">Acceso VIP Restringido v5.0</p>
        </div>
        ...
        function refreshLives() {
            fetch('/api/lives').then(r => r.json()).then(data => {
                ...
                if (data.lives.length === 0) {
                    list.innerHTML = '<p style="color: #999;">Sin LIVES detectadas</p>';
                    return;
                }
                list.innerHTML = data.lives.map(live => `
                    <div class="live-item">
                        <div class="live-cc">${live.cc}</div>
                        <div style="font-size: 12px; margin-top: 5px;">
                            Status: <strong>${live.status}</strong> | Bank: <strong>${live.bank}</strong>
                        </div>
...
@require_login
def api_lives():
    """Obtiene LIVES detectadas"""
    recent_lives = lives_mgr.get_recent_lives(limit=10)
    return jsonify({'lives': recent_lives}), 200

@app.route('/api/logs')
@require_login
def api_logs():
    """Obtiene logs del sistema"""
    logs = logger.get_recent(limit=50)
    return jsonify({'logs': logs}), 200

@app.route('/api/owner/lives')
@require_role('owner')
def api_owner_lives():
    """Panel OWNER: obtener todos los LIVES"""
    all_lives = lives_mgr.get_all_lives()
    return jsonify({'lives': all_lives}), 200

@app.route('/api/owner/clear-lives', methods=['POST'])
@require_role('owner')
def api_owner_clear_lives():
    """Panel OWNER: limpiar LIVES"""
    lives_mgr.clear_lives()
    logger.add("🔥 Panel OWNER: LIVES limpiadas")
...
def api_health():
    ...
    return jsonify({
        'status': 'healthy',
        'version': '5.0 Elite',
        'component': 'Scrapper Team RedCards'
    }), 200

async def start_telegram_client():
    """Inicia el cliente de Telegram en background"""
    try:
        logger.add("📱 Iniciando cliente Telegram...")
        await client.start()
        logger.add("✅ Cliente Telegram conectado")
...
def telegram_thread():
    """Ejecuta Telegram en un hilo separado"""
...
if __name__ == '__main__':
    ...
    telegram_t.start()
    logger.add("✅ Hilo Telegram iniciado")

    logger.add(f"🚀 Servidor Flask iniciando en puerto {PORT}")
